...
Windows 10/11 x64，推荐使用 Python 3.14 x64。

1. 双击 `install.bat`。脚本会创建隔离的 `.venv` 并安装 NumPy、Pillow 和 RealSense 2.58.1 Python 绑定；仓库为 Python 3.14 x64 附带了 RealSense wheel。
2. 双击 `run.bat` 启动软件。
3. 初次使用保持“仿真模式”勾选，确认界面、测距和保存流程正常。
4. 实机使用时将相机直接连接 USB 3.x 端口，并建议先用 RealSense Viewer 验证设备。
...
`debug/LIBSDepthCamera_Debug_win-x64.zip` 是可直接解压运行的控制台调试版，不要求目标电脑预装 Python。完整解压后双击包内“启动调试版.bat”；运行日志保存在 `logs`。详细步骤见 `debug/DEBUG_TEST_GUIDE.txt`。
